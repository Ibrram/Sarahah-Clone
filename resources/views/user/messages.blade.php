...
@section('content')
    <section class="text-gray-500 bg-gray-900 body-font">
        <div class="container px-5 py-20 mx-auto">
            <h1 class="text-3xl font-medium title-font text-white mb-3 text-center">Your Messages</h1>
            <h3 class="mb-12 text-center">Your link <a class="text-blue-500 font-bold" href="http://{{ Request()->getHost() . '/' . Auth()->user()->username }}">{{ Request()->getHost() . '/' . Auth()->user()->username }}</a></h3>
            <div class="flex flex-wrap -m-4">
                @if($messages->count() > 0)
                    ...
                    <div class="h-full bg-gray-800 p-8 rounded">
                        ...
                        <p class="leading-relaxed mb-6">{{ $message->message }}</p>
                    </div>
                </div>
                    @endforeach
                @else
